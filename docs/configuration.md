...
| Existing terminal does not activate | macOS activation uses AppleScript/TTY matching (Terminal.app/iTerm2) and may require Automation permission. Linux requires X11 and `xdotool`, plus a launch-time window ID or a unique emulator window. On Windows, a Windows Terminal window Biomelab launched is raised by its per-session window name, even after the tab title changes; a Windows Terminal session the OS default-terminal setting delegated a PowerShell fallback into cannot be named and reports a manual-switch message. A tracked classic PowerShell console is focused when its recorded visible HWND and exact title still match. Repeat Enter never launches a duplicate. Other macOS emulators and Wayland also require switching manually. |
