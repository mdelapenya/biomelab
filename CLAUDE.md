...
- **Help bar architecture**: Three independent help locations: (1) left panel footer `[a]dd [n]ew sandbox [x]rm` in `renderRepoList()`, (2) main card contextual help rendered below the main card border in `renderFixedTop()` — shows main-card-only actions (`c create`, `f fetch PR`, sandbox lifecycle) and adapts to sandbox status, uses `mainCardHelpActiveStyle` when cursor == 0 and `mainCardHelpFaintStyle` otherwise, (3) bottom help bar in `viewContent()` showing card-general and global actions (`navigate`, `open tab`, `editor`, `refresh`, `delete`, `pull`, `mouse`, `quit`). Do NOT mix main-card-only actions into the bottom bar or vice versa.
