...
func TestCredentialFillIsNoninteractive(t *testing.T) {
	...
	auth, err := credentialFill(context.Background(), "https://example.invalid/org/repo.git")
	if err != nil {
		t.Fatal(err)
	}
...
func TestCredentialFailureDoesNotExposeOutput(t *testing.T) {
	...
	_, err := credentialFill(context.Background(), "https://example.invalid/org/repo.git")
...
func TestCredentialFillHonorsCancelledContext(t *testing.T) {
	...
	cancel()
	_, err := credentialFill(ctx, "https://example.invalid/org/repo.git")
