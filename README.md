...
- **Delete worktrees** -- Press `d` on any linked worktree to delete it. A centered popup overlay shows what will happen (including which IDEs will be closed): press `y` to arm, then `Enter` to confirm. `Esc` cancels at any point. Open IDEs are killed, the worktree directory is removed, the branch is deleted, and stale metadata is pruned. The main worktree cannot be deleted.
